...
Tier 1: Direct CUDA Runtime (cudaMemcpy, cudaMemset)
Tier 2: Direct cuBLAS API (cublasGemmEx) - Supports Pascal & Ampere
Tier 3: libtorch Fallback (torch ops)
Tier 4: Communication (NCCL AllReduce, AllGather, etc.)
...
def classify_kernel(kernel: Dict[str, Any]) -> int:
    ...
    Returns:
        ...
        2: cuBLAS (GEMM, GEMV)
        3: libtorch Fallback
        4: Communication (NCCL AllReduce, etc.)
    """
    ...
    if "nccl" in name or "allreduce" in name or "allgather" in name or "reducescatter" in name:
        return 4
...
def extract_libtorch_params(kernel: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if n.startswith("cuda") or n.startswith("culaunch"):
        ...
        params["operation"] = "elementwise"
    elif "rsqrt" in n or "cos_kernel" in n or "sin_kernel" in n or "neg_kernel" in n or "bitwise_not" in n:
        params["operation"] = "elementwise"
...
def classify_all_kernels(
    ...
    print(
        f"  Tier 1 (CUDA Runtime): {tier_counts[1]} unique, {tier_invocations[1]} invocations ({100.0 * tier_invocations[1] / total_inv:.1f}%)"
    )
    print(
        f"  Tier 2 (cuBLAS):       {tier_counts[2]} unique, {tier_invocations[2]} invocations ({100.0 * tier_invocations[2] / total_inv:.1f}%)"
    )
    print(
        f"  Tier 3 (libtorch):     {tier_counts[3]} unique, {tier_invocations[3]} invocations ({100.0 * tier_invocations[3] / total_inv:.1f}%)"
    )
    print(
        f"  Tier 4 (Communication): {tier_counts[4]} unique, {tier_invocations[4]} invocations ({100.0 * tier_invocations[4] / total_inv:.1f}%)"
    )
