...
# Pin this process to one GPU before any CUDA/torch init. Torchrun sets LOCAL_RANK;
# some setups (MPS, env not forwarded) cause workers to see 0 GPUs otherwise.
_local_rank = os.environ.get("LOCAL_RANK")
if _local_rank is not None:
    _devices = os.environ.get("CUDA_VISIBLE_DEVICES", "0,1").strip()
    _device_list = [x.strip() for x in _devices.split(",") if x.strip()]
    if _device_list:
        _idx = int(_local_rank) % len(_device_list)
        os.environ["CUDA_VISIBLE_DEVICES"] = _device_list[_idx]
...
def main() -> int:
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model, dtype=torch.float16, low_cpu_mem_usage=True)
    model.eval()
    model.to(device)
